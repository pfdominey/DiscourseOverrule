...
def run_metusalem_control(subject, bundle_name, control):
    ...
    for instances in range(reps):
      ...
      print("************ ens._W count before: ", esn_w_count)

      # use the connectome to change the weights (_W) of the reservoir
      n_c_gain  = 2
      esn._W = connectome_weights * esn._W * n_c_gain 
      esn_w_count = np.count_nonzero(esn._W)
      connectome_count=np.count_nonzero(connectome_weights)
      
      
      
      
      print("**************************************** testing *************")
      print(" ******** Connectome non-zeros: ", connectome_count, "esn._W non-zeros: ",esn_w_count)
      ...
      if bundle_name != "None":
        ...
        # ok we will remove a bundle or an equivalent number of fibres
        
        print("removing", bundle_name, "control: ", control)
        ...
        ratio=bundle_count/connectome_count
        
        if control:
            
            # remove a random number of connections in the same proportion
            
            # this is where we were testing random removal 
            for i in range (1000):
                for j in range(1000):
                    if(rand.random())<ratio:
                        esn._W[i,j]=0
            esn_w_minus_count = np.count_nonzero(esn._W)
            print("**************  Connectome minus control percent esn._W non-zeros: ",esn_w_minus_count)
            print("percentage: ", 1-(esn_w_minus_count/esn_w_count), "vs:", ratio)
    ...
    filename = 'connectome_inference_control/subject_%s_path_%s_control_%s.npy' % (subject,bundle_name,control)
